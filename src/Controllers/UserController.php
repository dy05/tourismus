...
class UserController extends Controller{
    ...
    public function index()
    {
         ...
         */
        $p['values'] = $this->User->getLast($this->db);
        var_dump($p['values']);
        die();
        /**
    ...
    public function edit($db, $id=null)
    {

 /*       $users = $db->prepare("INSERT INTO users (name) VALUES(:name)", [':name' => 'hahahahaha']);
        var_dump($users);
        die();
 ...
 */       $p = array();
        if(!is_null($id)){
            $idi = $id[2];
            $user = $this->User->getLast($this->db);
//            $user = $this->User->getLast($this->db, $id[2]);
//            var_dump($user->name);
            var_dump($user);
            die();
        }
